...
此外，也可以使用 [OrcaRouter](https://www.orcarouter.ai) 这一 OpenAI 兼容网关来调用大模型：在[OrcaRouter](https://www.orcarouter.ai) 获取 API key 后，使用 `--orcarouter_api_key` 参数即可，产线会将对话与向量化请求统一路由到 OrcaRouter。
